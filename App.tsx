...
export default function App() {
  ...
  const startSimulation = async (scenario: SimulationScenario) => {
    ...
    try {
      if (!genAI) {
        throw new Error("Chybi promenna prostredi VITE_GEMINI_API_KEY.");
      }
      ...
      setSimulationError(error instanceof Error ? error.message : "Nepodarilo se spustit simulaci.");
    } finally {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (text) {
        ...
        throw new Error("Prazdna odpoved.");
      }
    } catch (error: any) {
      console.error("Chat error:", error);
      setSimulationError(error instanceof Error ? error.message : "Chyba pri generovani odpovedi.");
    } finally {
      setIsSimulationLoading(false);
    }
  };
